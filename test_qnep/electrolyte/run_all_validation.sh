#!/bin/bash
# Run the full electrolyte validation workflow.
...
run_step "step4 LAMMPS NEP5 vs MatPL ref-result, threshold=1e-3" \
  run_in_dir "$ROOT_DIR/lmps-infer" sbatch --wait job_lmps_compare.sh \
    --structure all --kspace both --threshold 1e-3 --keep-going

run_step "step4 LAMMPS NEP4 vs MatPL ref-result, threshold=1e-3" \
  run_in_dir "$ROOT_DIR/lmps-infer-nep4" sbatch --wait job_lmps_compare.sh \
    --structure all --kspace both --threshold 1e-3 --keep-going
...
lmps_nep5_summary=$ROOT_DIR/lmps-infer/compare_results/compare_summary.csv
lmps_nep4_summary=$ROOT_DIR/lmps-infer-nep4/compare_results/compare_summary.csv
EOF
