...
std::tuple<std::vector<int>, std::vector<int>, std::vector<float>>PairMATPL::convert_dim(bool is_build_neighbor){
    int nlocal = atom->nlocal;
...
void PairMATPL::compute(int eflag, int vflag)
{
    ...
    //   calculate the error of the force
        ...
        if (is_build_neighbor) {
            cpu_potential_per_atom.resize(list->inum, 0.0f);
            cpu_force_per_atom.resize(n_all * 3, 0.0f);
            cpu_total_virial.resize(6, 0.0f);
        }
        // else { 
        //     // 使用 memset 快速置零 不需要置零，因为copy from device 会直接覆盖
        //     std::memset(cpu_potential_per_atom.data(), 0, cpu_potential_per_atom.size() * sizeof(float));
        //     std::memset(cpu_force_per_atom.data(), 0, cpu_force_per_atom.size() * sizeof(float));
        //     std::memset(cpu_total_virial.data(), 0, cpu_total_virial.size() * sizeof(float));
        // }
