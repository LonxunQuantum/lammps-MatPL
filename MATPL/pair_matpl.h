...
namespace LAMMPS_NS {
    class PairMATPL : public Pair {
        public:
            ...
            std::tuple<std::vector<int>, std::vector<int>, std::vector<float>> convert_dim(bool is_build_neighbor=false);
        ...
        private:
            ...
            std::vector<float> cpu_potential_per_atom;
            std::vector<float> cpu_force_per_atom;
            std::vector<float> cpu_total_virial;
            
            std::vector<float> position_cpu; // for nep gpu optim
