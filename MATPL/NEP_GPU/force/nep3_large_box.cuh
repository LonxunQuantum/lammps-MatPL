...
static __device__ __inline__ float atomicAdd(float* address, float val) {
    int* address_as_int = (int*)address; // 将 float 指针转换为 int 指针
    int old = *address_as_int, assumed;
    do {
        assumed = old;
        // 使用 atomicCAS 进行比较和交换
        old = atomicCAS(address_as_int, assumed, __float_as_int(val + __int_as_float(assumed)));
    } while (assumed != old); // 如果 assumed 和 old 不相等，说明其他线程修改了值，需要重试
    return __int_as_float(old); // 返回旧值
}
#endif
  ...
  if (n1 < N) {
    ...
    for (int i1 = 0; i1 < g_NN[n1]; ++i1) {
      ...
      if (n2 >= N) {
        ...
        s_fy += f12[1];
        s_fz += f12[2];
        atomicAdd(&g_fx[n2], -f12[0]);// ghost atom
  ...
  if (n1 >= N1 && n1 < N) {
    ...
    for (int i1 = 0; i1 < g_NN_angular[n1]; ++i1) {
      ...
      if (n2 < N) {
        ...
        s_fy += f12y;
        s_fz += f12z;
        atomicAdd(&g_fx[n2], -f12x);// ghost atom
        atomicAdd(&g_fy[n2], -f12y);
        atomicAdd(&g_fz[n2], -f12z);
...
__global__ void calculate_total_virial(const float* virial, float* total_virial, int N) {
    __shared__ float shared_virial[6 * 64]; // 使用共享内存存储部分和
    int tid = threadIdx.x;
    int bid = blockIdx.x;
  ...
  if (n1 < N) {
    ...
    for (int i1 = 0; i1 < g_NN[n1]; ++i1) {
      ...
      if (n2 >= N) {
        ...
        s_fy += f12[1];
        s_fz += f12[2];
        atomicAdd(&g_fx[n2], f21[0]);// ghost atom
        atomicAdd(&g_fy[n2], f21[1]);
        atomicAdd(&g_fz[n2], f21[2]);
