...
if [ "$1" = "-h" ] || [ "$1" = "--help" ]; then
    echo "This script is used to copy the MatPL-2026.3 lammps CPU interface files to the corresponding lammps directory:"
    echo "bash kknep-patch-cpu.sh lammps/rootdir"
    exit 0
fi
...
# CPU-only build: copy only CPU files, skip nep_gpu/ (GPU binaries not needed)
echo "Skipping nep_gpu/ directory (not needed for CPU-only build)."
