...
#include "nep_gpu_loader.h"
#include <utility>

namespace LAMMPS_NS {

template<class DeviceType>
class PairNEPKokkos : public PairNEP, public KokkosBase {
  public:
    enum class ReverseCommMode { NONE, FORCE_DEVI, HEATFLUX };
    ...
    std::tuple<double, double, double, double, double, double> calc_max_error(const int* ilist, const F_FLOAT* h_full_f, const F_FLOAT* h_full_e, const int num_ff, const int inum, const int nlocal, const int nall, const int rank);
    typename AT::t_efloat_1d device_eatom_view() const { return d_eatom; }
    Kokkos::View<double*[9], Kokkos::LayoutRight, DeviceType> device_cvatom_view() const { return d_cvatom; }
    bool has_device_heatflux_views() const { return d_eatom.extent(0) > 0 && d_cvatom.extent(0) > 0; }
    void update_heatflux_reverse();
    auto heatflux_reverse_view() const { return k_heatflux_reverse.template view<DeviceType>(); }
  protected:
    bool need_active_matpl_heatflux_device(bigint);
    ...
    int global_nall = 0;
    int global_nlocal = 0;
    std::vector<NepModelLoader> nep_gpu_models;  // NEP model instance (dlopen-loaded)
    ...
    DAT::tdual_xfloat_1d k_heatflux_reverse;
    int heatflux_reverse_nmax = 0;
    ReverseCommMode reverse_comm_mode = ReverseCommMode::NONE;
