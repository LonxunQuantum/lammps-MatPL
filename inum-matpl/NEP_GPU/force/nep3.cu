...
void NEP3::set_atom_type_map(int type_nums, const int* type_list){
...
void NEP3::rest_nep_data(int inum, int n_local, int n_all, int max_neighbor) {
  ...
  if (atom_nums != inum) {
    atom_nums = inum;
    lmp_data.ilist.resize(atom_nums); // 在只使用部分原子计算时，inum < nlocal
  }
 
  if (atom_nlocal != n_local) { 
    atom_nlocal = n_local;
    lmp_data.numneigh.resize(n_local);
    lmp_data.firstneigh.resize(n_local*max_neighbor);

    nep_data.NN_radial.resize(n_local);
    nep_data.NL_radial.resize(n_local * paramb.MN_radial);
    nep_data.NN_angular.resize(n_local);
    nep_data.NL_angular.resize(n_local * paramb.MN_angular);

    nep_data.f12x.resize(n_local * paramb.MN_angular);
    nep_data.f12y.resize(n_local * paramb.MN_angular);
    nep_data.f12z.resize(n_local * paramb.MN_angular);

    nep_data.potential_per_atom.resize(n_local);
...
void NEP3::compute_large_box_optim(
  ...
  // checkMemoryUsage(0); 
  // printf("===rankid %d N %d nlocal %d n_all %d NM %d paramb.MN_radial %d MN_angular %d isbuild %d\n", device_id, N, nlocal, n_all, NM, paramb.MN_radial, paramb.MN_angular, is_build_neighbor);
  ...
  gpu_sort_neighbor_list<<<nlocal, paramb.MN_radial, paramb.MN_radial * sizeof(int)>>>(
    nlocal, nep_data.NN_radial.data(), nep_data.NL_radial.data());
  CUDA_CHECK_KERNEL 
  ...
  // }
  // if (device_id==0) {
  // std::vector<double> tmp_potential_per_atom(nlocal);
  // std::vector<double> tmp_force_2b(n_all*3);
  // nep_data.potential_per_atom.copy_to_host(tmp_potential_per_atom.data());
  // nep_data.force_per_atom.copy_to_host(tmp_force_2b.data());
  // for (int ii = 0; ii < N; ii++) {
  //   printf("radial ghost ei[%d] = %f f = [%f %f %f]\n", 
  //     ii, tmp_potential_per_atom[ii], tmp_force_2b[ii], tmp_force_2b[ii + n_all], tmp_force_2b[ii + n_all * 2]);
  // }

  // for (int ii = N; ii < n_all; ii++) {
  //   printf("radial ghost i>N f[%d] = [%f %f %f]\n", 
  //     ii, tmp_force_2b[ii], tmp_force_2b[ii + n_all], tmp_force_2b[ii + n_all * 2]);
  // }
  // }
  // if (1) {
  // std::vector<double> tmp_potential_per_atom(nlocal);
  ...
  // nep_data.potential_per_atom.copy_to_host(tmp_potential_per_atom.data());
  // nep_data.force_per_atom.copy_to_host(tmp_force_2b.data());
  // printf("===done copytmp raidal rankid %d N %d nlocal %d n_all %d NM %d paramb.MN_radial %d MN_angular %d isbuild %d\n", device_id, N, nlocal, n_all, NM, paramb.MN_radial, paramb.MN_angular, is_build_neighbor);
  // }
  // printf("===done calculate 2b rankid %d N %d nlocal %d n_all %d NM %d paramb.MN_radial %d MN_angular %d isbuild %d\n", device_id, N, nlocal, n_all, NM, paramb.MN_radial, paramb.MN_angular, is_build_neighbor);
  ...
  // nep_data.potential_per_atom.copy_to_host(tmp_potential_per_atom.data());
  // nep_data.force_per_atom.copy_to_host(tmp_force_2b.data());
  // printf("===done copytmp partial rankid %d N %d nlocal %d n_all %d NM %d paramb.MN_radial %d MN_angular %d isbuild %d\n", device_id, N, nlocal, n_all, NM, paramb.MN_radial, paramb.MN_angular, is_build_neighbor);
  // }

  // printf("===done calculate 3bhalf rankid %d N %d nlocal %d n_all %d NM %d paramb.MN_radial %d MN_angular %d isbuild %d\n", device_id, N, nlocal, n_all, NM, paramb.MN_radial, paramb.MN_angular, is_build_neighbor);
  ...
  // nep_data.force_per_atom.copy_to_host(tmp_force_2b.data());
  // for (int ii = 0; ii < N; ii++) {
  //   printf("angular ghost ei[%d] = %f f = [%f %f %f]\n", 
  //     ii, tmp_potential_per_atom[ii], tmp_force_2b[ii], tmp_force_2b[ii + n_all], tmp_force_2b[ii + n_all * 2]);
  // }

  // for (int ii = N; ii < n_all; ii++) {
  //   printf("angular ghost i>N f[%d] = [%f %f %f]\n", 
  //     ii, tmp_force_2b[ii], tmp_force_2b[ii + n_all], tmp_force_2b[ii + n_all * 2]);
  // }
  // }
  // std::vector<double> tmp_potential_per_atom(nlocal);
  ...
  // nep_data.potential_per_atom.copy_to_host(tmp_potential_per_atom.data());
  // nep_data.force_per_atom.copy_to_host(tmp_force_2b.data());
  // printf("===done copytmp many rankid %d N %d nlocal %d n_all %d NM %d paramb.MN_radial %d MN_angular %d isbuild %d\n", device_id, N, nlocal, n_all, NM, paramb.MN_radial, paramb.MN_angular, is_build_neighbor);
  // }
  // printf("===done calculate 3b rankid %d N %d nlocal %d n_all %d NM %d paramb.MN_radial %d MN_angular %d isbuild %d\n", device_id, N, nlocal, n_all, NM, paramb.MN_radial, paramb.MN_angular, is_build_neighbor);
  ...
  CUDA_CHECK_KERNEL
  cudaDeviceSynchronize();
  // printf("===done calculate rankid %d N %d nlocal %d n_all %d NM %d paramb.MN_radial %d MN_angular %d isbuild %d\n", device_id, N, nlocal, n_all, NM, paramb.MN_radial, paramb.MN_angular, is_build_neighbor);

  // std::vector<double> tmp_potential_per_atom(nlocal);
  // std::vector<double> tmp_force_2b(n_all*3);
  // nep_data.potential_per_atom.copy_to_host(tmp_potential_per_atom.data());
  // nep_data.force_per_atom.copy_to_host(tmp_force_2b.data());
  // printf("===done copytmp rankid %d N %d nlocal %d n_all %d NM %d paramb.MN_radial %d MN_angular %d isbuild %d\n", device_id, N, nlocal, n_all, NM, paramb.MN_radial, paramb.MN_angular, is_build_neighbor);
  cudaDeviceSynchronize();
  ...
  // printf("===done rankid %d N %d nlocal %d n_all %d NM %d paramb.MN_radial %d MN_angular %d isbuild %d\n", device_id, N, nlocal, n_all, NM, paramb.MN_radial, paramb.MN_angular, is_build_neighbor);
