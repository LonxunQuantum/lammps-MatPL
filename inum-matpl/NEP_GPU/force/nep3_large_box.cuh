  ...
  int* g_NL_angular)
{
  ...
  if (n1 < N) {// 用的inum值，而不是local
    int count_radial = 0;
    int count_angular = 0;
    int atomi = g_ilist[n1]; //atom i
    int jnum = g_numneigh[atomi];
    for (int jj = 0; jj < jnum; jj++) {
      int n2 = g_firstneigh[atomi * NM + jj]; // i的邻居是 n1 指向的位置
      double x12 = g_x[n2] - g_x[atomi];
      double y12 = g_y[n2] - g_y[atomi];
      double z12 = g_z[n2] - g_z[atomi];
      float distance_square = float(x12 * x12 + y12 * y12 + z12 * z12);

      if (distance_square < paramb.rc_radial * paramb.rc_radial) {
        g_NL_radial[count_radial * nlocal + atomi] = n2;
        count_radial++;
      }
      
      if (distance_square < paramb.rc_angular * paramb.rc_angular) {
        g_NL_angular[count_angular * nlocal + atomi] = n2;
        count_angular++;
      }
    } //for
    g_NN_radial[atomi] = count_radial;
  ...
  float* g_sum_fxyz)
{
  ...
  if (n1 < N) {
    ...
    int t1 = type_map[g_type[atomi]-1];
    float q[MAX_DIM] = {0.0f};
    double x1 = g_x[atomi];
    double y1 = g_y[atomi];
    double z1 = g_z[atomi];
    // get radial descriptors
    for (int i1 = 0; i1 < g_NN_radial[atomi]; ++i1) {
      int n2 = g_NL_radial[i1 * nlocal + atomi];
      int t2 = type_map[g_type[n2]-1];
    ...
    g_pe[atomi] += F;
    for (int d = 0; d < annmb.dim; ++d) {
      g_Fp[d * nlocal + atomi] = Fp[d] * paramb.q_scaler[d];
  ...
  if (n1 < N) {
    ...
    for (int i1 = 0; i1 < g_NN[atomi]; ++i1) {
      ...
      for (int n = 0; n <= paramb.n_max_radial; ++n) {
        ...
        float tmp12 = g_Fp[atomi + n * nlocal] * gnp12 * d12inv;
        float tmp21 = 0;
        if (n2 >= nlocal) {
          for (int d = 0; d < 3; ++d) {
            f12[d] += tmp12 * r12[d];
    ...
    g_fy[atomi] += s_fy;
    g_fz[atomi] += s_fz;
    // printf("2bf fxyz[n1 %d -> %d] %f %f %f\n", n1, atomi, s_fx, s_fy, s_fz);
    // save virial
    // xx xy xz    0 3 4
    // yx yy yz    6 1 5
    // zx zy zz    7 8 2
  ...
  )
{
  ...
  if (n1 < N) {
    ...
    for (int d = 0; d < paramb.dim_angular; ++d) {
      Fp[d] = g_Fp[(paramb.n_max_radial + 1 + d) * nlocal + atomi];
    }
  ...
  if (n1 >= N1 && n1 < N) {
    ...
    for (int i1 = 0; i1 < g_NN_angular[atomi]; ++i1) {
      ...
      if (n2 < nlocal) {
        ...
        //反推这里不对，因为NN不是中心原子索引，这里要找的是n2对应的nn
  ...
  double* g_pe)
{
  ...
  if (n1 < N) {
    ...
    double y1 = g_y[atomi];
    double z1 = g_z[atomi];
    int type1 = type_map[g_type[atomi] - 1];
    float zi = zbl.atomic_numbers[type1];
    float pow_zi = pow(zi, 0.23f);
    for (int i1 = 0; i1 < g_NN[atomi]; ++i1) {
      int n2 = g_NL[atomi + nlocal * i1];
      int type2 = type_map[g_type[n2] - 1];
      double x12double = g_x[n2] - x1;
      double y12double = g_y[n2] - y1;
      double z12double = g_z[n2] - z1;
      float r12[3] = {float(x12double), float(y12double), float(z12double)};
